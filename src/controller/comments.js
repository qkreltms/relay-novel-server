module.exports = (pool) => {
    ...
    const runQuery = async (errHandlerCallback) => {
      try {
        if (userId) {
          ...
          const sql = `SELECT id, text, userId, updatedAt, updatedAt, \`like\`, dislike FROM comments WHERE roomId = ? AND isDeleted = ? ORDER BY createdAt DESC LIMIT ${skip}, ${limit}`
  ...
  api.post('/likedislikes', (req, res) => {
    ...
    const userId = req.body.userId
    console.log(commentId, roomId)
    console.log(isLike, userId)
